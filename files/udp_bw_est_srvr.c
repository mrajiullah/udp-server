...
// Information used by each threads
...
uint64_t generate_udp_traffic(struct thread_info *thread_info){
    ...
    sent_packet=0;
        ...
        // we want to make sure that we dont send anything for the rest if the second
        // time_left returns number of milliseconds left fro 1000 ms
        time_left=1000-((t1_p.tv_sec - t0_p.tv_sec) * 1000 + (double)(t1_p.tv_usec - t0_p.tv_usec) / 1000);
    if (time_left>0){
        //usleep((1000-time_left)*1000);
        tim.tv_sec = time_left / 1000;
        tim.tv_nsec = fmod(time_left,1000) * NANOSLEEP_MS_MULTIPLIER;
        nanosleep(&tim, &tim2);
        }
    sent_burst++;
